...
export default async (req, res, next) => {
	...
	if (usuario && cliente && usuario.cliente_id && usuario.cliente_id == cliente_id) {
		...
		if (usuario.fechaUltNaveg < fechaUltNaveg) {
			const diasNaveg = ++cliente.diasNaveg;
			req.session.usuario = {...req.session.usuario, fechaUltNaveg, diasNaveg};
			req.session.cliente = {...req.session.cliente, fechaUltNaveg, diasNaveg};
			await baseDatos.actualizaPorId("usuarios", usuario.id, {fechaUltNaveg, diasNaveg});
		}
	...
	// Usuario: Lo obtiene de su cookie 'mail'
	...
	// Cliente: 2. Lo obtiene de la cookie y si no existe el registro en la BD, elimina la cookie
	...
	if (!cliente) {
		...
		// Crea un nuevo 'cliente_id' y crea la cookie
	...
	if (cliente.fechaUltNaveg < fechaUltNaveg) {
		...
		if (usuario) usuario = {...usuario, fechaUltNaveg, diasNaveg};
	}
