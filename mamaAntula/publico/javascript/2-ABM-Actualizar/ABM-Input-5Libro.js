...
window.addEventListener("load", async () => {
	...
	// Eventos drag & drop
	DOM.areaSoltar.addEventListener("drop", async (e) => (archLibro = await nuevaImg(e.dataTransfer.files, DOM.vistaImagen)));
	otrosEventos(DOM);

	// Eventos del formulario
	DOM.inputImagen.addEventListener("change", async () => (archLibro = await nuevaImg(DOM.inputImagen.files, DOM.vistaImagen)));
